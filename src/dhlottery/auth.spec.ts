...
describe("DHLottery Authentication", () => {
  ...
  beforeEach(() => {
    ...
    mockHttpClient = {
      cookies: {
        // Pre-set JSESSIONID to satisfy initSession verification
   ...
   * Returns session init response, login response, and redirect response
   */
  ...
  describe("TEST-AUTH-001: Authenticate with valid credentials", () => {
    it("should successfully authenticate with valid credentials", async () => {
      ...
      expect(sessionCall[0]).toContain("/gameResult.do?method=byWin&wiselog=H_C_1_1");
