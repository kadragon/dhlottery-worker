...
- [ ] [enhance] `aggregateWindow` `maxPages(200)` 소진 시 `fetched < total`이면 부분 집계 — truncation 경고/실패 신호. (윈도당 ≤90d라 발생 가능성 낮으나 backstop 무음) (source: antigravity, pr-review-toolkit:review-pr)
- [ ] [enhance] `LEDGER_START_DATE` 포맷(YYYYMMDD) 검증 — 무효값이면 `DefaultLedgerStartDate` fallback + 경고 로그. (source: antigravity)
